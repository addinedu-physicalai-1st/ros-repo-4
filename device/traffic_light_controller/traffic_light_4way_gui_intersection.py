...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QFormLayout, QFrame, QSizePolicy
)
...
class IntersectionWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawLine(cx - 55, cy - road_w // 2 + 22, cx + 55, cy - road_w // 2 + 22)   # top
        p.drawLine(cx - 55, cy + road_w // 2 - 22, cx + 55, cy + road_w // 2 - 22)   # bottom
        p.drawLine(cx - road_w // 2 + 22, cy - 55, cx - road_w // 2 + 22, cy + 55)   # left
        p.drawLine(cx + road_w // 2 - 22, cy - 55, cx + road_w // 2 - 22, cy + 55)   # right
...
class MainWindow(QWidget):
    ...
    def build_ui(self):
        ...
        for i in range(4):
            ...
            conn = QLabel("미연결")
            conn.setStyleSheet("font-weight:bold;color:red;")
        ...
        for lab in [self.system_mode_label, self.active_green_label, self.phase_label]:
            lab.setStyleSheet("font-size:15px;font-weight:bold;")
        ...
        tabs.addTab(self.auto_tab, "자동")
        tabs.addTab(self.manual_tab, "수동/반자동")
        left_wrap.addWidget(tabs, 1)
        ...
        self.log_label.setMinimumHeight(220)
        self.log_label.setStyleSheet("background:#f8f9fb;border-radius:10px;padding:10px;")
        log_layout.addWidget(self.log_label)
        left_wrap.addWidget(log_group)
    ...
    def build_manual_tab(self):
        ...
        note = QLabel(
            "반자동: 현재 초록불이 있으면 Y를 거친 뒤 R로 바꾸고, 목표 신호등을 G로 전환합니다."
        )
    ...
    def append_log(self, msg: str):
        ...
        lines = [] if old == "로그 없음" else old.splitlines()
        lines.append(msg)
